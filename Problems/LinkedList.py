class node:
  ...
  def asJson(self):
    return dict(value=self.value,nextNode= self.nextNode.asJson() if self.nextNode is not None else self.nextNode )
...
class LinkedList:
  ...
  def AsJson(self):
    return dict(head=self.head.asJson(),tail=self.tail.asJson(),length=self.length)
